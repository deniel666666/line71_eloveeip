...
class ProductApiController extends Controller
{
    ...
    public function setLayout(Request $request){
        ...
        // if ($style == 6) {
        //     $productType_db = $this->mainRepository->getProductType($prod_id);
        //     $productTypeIds = array_column($productType_db, 'prod_type_id');
            
        //     if (count($productTypeIds) > 1) {
        //         foreach (array_slice($productTypeIds, 1) as $id) {
        //             DB::beginTransaction();
        //             $checkVal = $this->mainRepository->deleteOne('ProductType',  [['prod_type_id','=', $id]]);
        //             if ($checkVal > 0) {
        //                 DB::rollback();
        //             } else {
        //                 DB::commit();
    ...
    public function copy(Request $request){
        ...
        $products = $this->mainRepository->getProducts($post_data);
        $products = $products->toArray();

        $includeIds = $request->post('includeIds');
        $includeIds = json_decode($includeIds ?? '[]');
        $includeIds = array_reverse($includeIds);
        foreach ($includeIds as $prod_id) {
            $product = (array)DB::table('product')->where('prod_id', $prod_id)->first();
            unset($product['prod_id']);

            $product['prod_order'] = -1;
            $new_prod_id = $this->mainRepository->addProduct($product);
            // dd($new_prod_id);

            /*複製商品圖檔*/
            $source = $this->publicFilePath.$prod_id;
            $destination = $this->publicFilePath.$new_prod_id;
            AppHelper::copyFolder($source, $destination);

            /*複製分類*/
            $product_category = DB::table('product_category')->where('prod_id', $prod_id)->get()->toArray();
            foreach ($product_category as $key => $value) {
                $product_category[$key] = (array)$value;
                unset($product_category[$key]['prod_cate_id']);
                $product_category[$key]['prod_id'] = $new_prod_id;
            }
            // dump($product_category);
            DB::table('product_category')->insert($product_category);

            /*複製說明*/
            $product_describe = DB::table('product_describe')->where('prod_id', $prod_id)->get()->toArray();
            foreach ($product_describe as $key => $value) {
                $product_describe[$key] = (array)$value;
                unset($product_describe[$key]['prod_describe_id']);
                $product_describe[$key]['prod_id'] = $new_prod_id;
            }
            // dump($product_describe);
            DB::table('product_describe')->insert($product_describe);

            /*複製圖片*/
            $product_img = DB::table('product_img')->where('prod_id', $prod_id)->get()->toArray();
            foreach ($product_img as $key => $value) {
                $product_img[$key] = (array)$value;
                unset($product_img[$key]['prod_img_id']);
                $product_img[$key]['prod_id'] = $new_prod_id;
            }
            // dump($product_img);
            DB::table('product_img')->insert($product_img);

            /*複製屬性*/
            $product_property = DB::table('product_property')->where('prod_id', $prod_id)->get()->toArray();
            foreach ($product_property as $key => $value) {
                $product_property[$key] = (array)$value;
                unset($product_property[$key]['prod_type_id']);
                $product_property[$key]['prod_id'] = $new_prod_id;
            }
            // dump($product_property);
            DB::table('product_property')->insert($product_property);

            /*複製包裝*/
            $product_type = DB::table('product_type')->where('prod_id', $prod_id)->get()->toArray();
            foreach ($product_type as $key => $value) {
                $product_type[$key] = (array)$value;
                unset($product_type[$key]['prod_type_id']);
                $product_type[$key]['prod_id'] = $new_prod_id;
            }
            // dump($product_type);
