...
class OrderApiController extends Controller
{
	...
	public function page(Request $request){
		...
		$res = $this->orderRepository->showPage($pageData);

		$pageCount = $this->orderRepository->orderCount($pageData);
	...
	public function updatePayStatus(Request $request,$orderId){
		...
		$res = $this->orderRepository->update($orderId,$updData);
	...
	public function trash(Request $request){
		...
		foreach($orderList as $olKey => $olValue){
			if ($olValue['selected']){
				$updData['trash_status'] = 1;
				$this->orderRepository->update($olValue['od_id'],$updData);
			}
		}
