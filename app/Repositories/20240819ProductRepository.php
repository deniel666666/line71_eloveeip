...
class ProductRepository extends CmsTypeTemplateRepository{
	...
	public function getOneProduct_sql($productId){
		$res = $this->mainModel->with(['lang'])
				->select('prod_id','lang_id','prod_name','prod_order','product.prod_subtitle','product.prod_main_sku','product.prod_show_s_datetime','product.prod_show_e_datetime','prod_s_datetime','prod_e_datetime','prod_img','prod_img2','prod_img3','prod_status','prod_shelf','prod_sale','promote_prod','product_num', 'created_at','own_user as selectUser','style')
	...
	public function getProducts($cond){
		...
		if(isset($cond['user']) && $cond['user']['id'] > 2){
			...
			->where('product.own_user','=',$cond['user']['id']);
        ...
        if(isset($cond['cate_tag_id'])){
        	if($cond['cate_tag_id']!=''){
				$res =$this->mainModel->with([ 
						...
						'productDescribe',
						'productProperty',
						'productType' => function ($query) { return $query->orderBy('order_id','ASC')->orderBy('prod_type_id','DESC'); },
					])
	...
	public function getProductType($productId){
		return ProductTypeModel::select('prod_type_id','prod_type','type_price','type_sales_price','type_status','prod_sn', 'order_id')->where('prod_id','=',$productId)
