        ...
        <div>
            <ul class="footer-links container d-flex flex-wrap list-unstyled py-5 mb-0">
            @foreach($footerLink as $item)
                <li>
                    <a href=" {{$item->gallery_cont->link}}" target="_blank"
                        > {{$item->alt}}<span class="material-icons align-top ml-1">
                        open_in_new
                        </span>
                    </a>
                </li>
            ...
            <div class="d-md-flex flex-md-column list-unstyled">           
                <div class="contact-btns">
                    <button type="button" class="page-top-btn p-0 " title="回到上面">
                        <span class="material-icons go-to-top"> expand_less </span>
                    </button>

                    @if(isset($product))
                        @if(isset($product['propertyTag'][2]['prod_prop']))
                            @if(trim($product['propertyTag'][2]['prod_prop'])!='')
                                <a class="call-icon" href="tel:{{$product['propertyTag'][2]['prod_prop']}}" title="撥打電話" target="_blank">
                    ...
                    @else
                        @if($cmsPublic[0]['template']['tel1'])
                            <a class="call-icon" href="tel:{{$cmsPublic[0]['template']['tel1']}}" title="撥打電話" target="_blank">
                            <i class="bi bi-telephone"></i>
                            </a>
                        @endif
                    @endif
                    ...
                    @if(isset($product))
                        @if(isset($product['propertyTag'][3]['prod_prop']))
                            @if(trim($product['propertyTag'][3]['prod_prop'])!='')
                                <a href="{{$product['propertyTag'][3]['prod_prop']}}" title="LINE諮詢" target="_blank" class="line-icon">
                    ...
                    @else
                        @if($cmsPublic[0]['template']['line_link'])
                            <a href="{{$cmsPublic[0]['template']['line_link']}}" title="LINE諮詢" target="_blank"  class="line-icon">
                            <i class="bi bi-line"></i>
                            </a>
                        @endif
                    @endif
                ...
                <ul class="footer-item">
                    <li>
                        ...
                        <div class="fax">FAX : {{$cmsPublic[0]['template']['fax']}}</div>
                         @endif
                        @if($cmsPublic[0]['template']['email'])
                        <div class="fax">E-mail : <a href="mailto:{{$cmsPublic[0]['template']['email']}}">{{$cmsPublic[0]['template']['email']}}</a></div>
                        @endif
                        @if($cmsPublic[0]['cont']['text'])
                        <div class="item">
                            {!! $cmsPublic[0]['cont']['text'] !!}
                        </div>
                        @endif
                    </li>
                    ...
                    <li>
                        <ul class="community">
                            @if($cmsPublic[0]['template']['fb_link'])
                                <li class="fb_link"><a href="{{$cmsPublic[0]['template']['fb_link']}}" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-facebook" viewBox="0 0 16 16" style="margin-top: -6px;">
                ...
                <div class="container w1400">
                    <div class="row justify-content-between">
                        ...
                        <!-- <ul class="photonic-footer">
                            <div class="mr-2" style="color: #4887c2; opacity: 0.3;">傳訊光科技:</div>
