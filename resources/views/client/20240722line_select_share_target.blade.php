		                    ...
		                    if(Object.keys(data).length!=0){
								...
								for (let i = 0; i < template.length; i++) {
									for(let j =0 ;j < template[i].footer.contents.length;j++){
										...
										if(template[i].footer.contents[j].action != undefined){
											console.info(template[i].footer.contents[j]);
											if(template[i].footer.contents[j].action.label=='分享給朋友'){
												// template[i].footer.contents[j].action.uri='https://liff.line.me/'+liffID+'?rand='+rand
												template[i].footer.contents[j].action.uri='https://access.line.me/oauth2/v2.1/liff/shareTargetPicker?liffId=' + liffID;
									...
									if (card_array[i].body.contents[1]) {
										if (card_array[i].body.contents[1].contents) {
											if (card_array[i].body.contents[1].contents[0].text) {
												console.info(card_array[i].body.contents[1]);
