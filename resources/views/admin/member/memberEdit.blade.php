...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-6">
            <p>顯示狀態 ：<select ng-model="contCtrl.model.user.show_status" ng-options="item.id as item.label for item in contCtrl.showStatus" class="form-control"></select></p>
        </div>
        ...
        <div class="col-6">
            ...
            <!-- 新增 -->
            <input ng-if="contCtrl.model.user.id==0" type="text" ng-model="contCtrl.model.user.account" class="form-control" placeholder="" autocomplete="off">
            <!-- 編輯 -->
            <input ng-if="contCtrl.model.user.id!=0" type="text" ng-model="contCtrl.model.user.account" class="form-control" placeholder="" autocomplete="off" readonly="readonly">
        </div>
        <!-- 您的姓名 -->
        <div class="col-6">
        ...
        <!-- 修改密碼 -->
        <div class="col-6">
            <div ng-if="contCtrl.model.user.id==0">
                <!-- 新增 -->
        ...
        <!-- 購物車網址 -->
        <div class="col-6">
            <label for="modifyPassword">購物車網址</label>
            <input type="text" ng-model="contCtrl.model.user.shop_link" class="form-control" placeholder="若非B、C方案客戶則無需填寫(請包含 http:// 或 https://)" autocomplete="off">
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-12">
            <button type="button" class="col-12 btn btn-primary" ng-click="contCtrl.updateMemberInfo()">
                <span ng-if="contCtrl.model.user.id==0">新增資料</span>
                <span ng-if="contCtrl.model.user.id!=0">更新資料</span>
            </button>
        </div>
    </div>
    <br>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        app.controller('ContentController',['$http',function($http){
            ...
            self.listPage = "/admin/member";
			self.userInfoUrl = "/admin/api/member/Info/";
			...
			self.updateMemberInfo = function(){
				...
				if($('#password').val()){
					...
					self.model.user.password = $('#password').val();
				}
				...
				$http({
					...
					if (data.status == '200'){
                        ...
                        if(self.model.user.id==0){
                            location.href = self.listPage;
                        }
					}else{
                        $.toaster({ message : data.message, priority:'danger'});
                    }
				}).error(function(res){
                    $.toaster({ message : res.message, priority:'danger'});
				})//error
			}
        }])
    </script>
