		...
		<div class="row bg-light no-gutters pageHeader">
	        ...
	        <div class="col-lg-4  clearfix order-lg-2 order-4 mb-2">
	            <div class="float-lg-right">
	                ...
	                <span>
	                    <input class="use-form-control pdSpacing" style='width:50px;' type='text' ng-model="contCtrl.currentPage" ng-keydown="$event.keyCode === 13 && contCtrl.goto()">/@{{contCtrl.pageCount}}
	                </span>
	        ...
	        <div class="col-12 clearfix order-2 use-flexbox-align mb-2">
				<div class="float-right mr-5px">
					<select class="use-form-control maxWidth  w-80px" ng-model='contCtrl.selectLangItem' ng-change="contCtrl.changedListLang()" ng-options="option.lang_id as option.lang_word for option in contCtrl.langs">
						<option ng-selected="true" value="">全部</option>
					</select>
				</div>
		    ...
		    <table class="table table-bordered admin-table-rwd">
				...
				<tbody>
					<tr ng-repeat="(key,item) in contCtrl.galleryList">
						...
						<td data-th="詳細內容">
							<a class="editInfor w-100 d-inline-block" href="" ng-click="contCtrl.go_cms_edit(item.gallery_id)">編輯</a>
						</td>
						...
						<input type="hidden" ng-model="item.title">
					</tr>
				</tbody>
			</table>
...
@section('javascript')
    <script type="text/javascript">
        ...
        app.controller('ContentController',['$scope','$http',function($scope,$http){
			...
			self.editCmsPage	= "/{{$use_end}}/{{$content_table}}_cms/";
		    ...
		    self.go_cms_edit = function(item_id){
		    	location.href = self.editCmsPage + item_id;
		    }
				...
				if(!$scope.result){
					$http({
						...
						data: {
							...
							'sliderLink' 		: item.link,
							'sliderNote'		: item.note,
							'sliderAlt' 		: item.alt,
							'sliderOrder' 		: item.slider_order,
							'imgStatus' 		: item.img_status,
							'galleryId' 		: self.galleryId,  //galleryTypeId
							'galleryTypeId'		: self.galleryId,  //galleryTypeId
		    ...
		    self.prodStateSel = '';
		    self.selectLangItem = 1; //0 land all
