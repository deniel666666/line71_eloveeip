      ...
      <table class="table table-bordered admin-table-rwd form">
        <thead>
          <tr class="admin-tr-only-hide">
            ...
            <!-- <th class="w-140px" scope="col">主圖列表</th> -->
            <!-- <th class="w-50px" scope="col">型號</th> -->
            <th scope="col">主列表(編輯)</th>
            <!-- <th scope="col">編輯內容</th> -->
            <th scope="col">所屬分類<span class="mark-use">(將對應顯示到同「行銷主題」的EDM前台頁面中)</span></th>
            <th class="w-50px" scope="col">推薦</th>
            <!-- <th class="w-50px" scope="col">可否購買</th> -->
        ...
        <tbody>
          <tr ng-repeat="(key ,item) in contCtrl.items">
            ...
            <!-- <td data-th="主圖列表"> -->
              <!-- 主圖 -->
              <!-- <div class="adminImg-responsive-4halfBy3" ng-style="{'background-image': 'url('+item.prod_img+')'}"></div> -->
              <!-- 多圖上傳作 -->
            ...
            <!-- </td> -->
            <!-- <td data-th="型號"><span ng-bind="item.prod_main_sku"></span></td> -->
            <td data-th="主列表(編輯)"><a class="editInfor d-inline-block" href="/admin/product/edit/detail/{{$productNum}}/@{{item.prod_id}}"><span ng-bind="item.prod_name"></span></a></td>
            <!-- <td data-th="編輯內容"><a class="editInfor d-inline-block" href="/admin/product_cms/@{{item.prod_id}}">點我編輯</a></td> -->
            <td data-th="所屬分類">
              <p ng-repeat="tag_name in item.tag_with_layer_name" ng-bind="tag_name"></p>
            </td>
            <td data-th="推薦"><a href="" ng-click="contCtrl.changeStatus(item.prod_id,item.promote_prod,'promote_prod')"><span ng-bind="contCtrl.promote[item.promote_prod].name"></span></a></td>
            <!-- <td data-th="可否購買"><a href="" ng-click="contCtrl.changeStatus(item.prod_id,item.prod_sale,'prod_sale')"><span ng-bind="contCtrl.sale[item.prod_sale].name"></span></a></td> -->
