        ...
        <table class="table table-bordered admin-table-rwd form">
            ...
            <tbody>
                <tr ng-repeat="item in contCtrl.items">
                    ...
                    <td data-th="狀態" ng-disabled="@{{item.label_status == 1? 'item.itemColor =false': 'item.itemColor=true'}}" ng-class="{true: 'item_enable', false: 'item_disable'}[item.itemColor]">
                        <span ng-bind="item.label_status_name" ></span>
                    </td>
                    <td data-th="圖片" > 
                        <div class="adminImg-responsive-4halfBy3" ng-style="{'background-image': 'url('+item.label_img+')'}"></div>
                    </td>
                    <td data-th="ICON名稱">
                        <a class="editInfor w-100 d-inline-block" href="" data-toggle="modal" data-target="#itemData" ng-click='contCtrl.edit(item)' ng-bind="item.label_name"></a>
                    </td>
                    <td data-th="圖片說明" ng-bind="item.label_img_desc"></td>
                    <td data-th="內容" ng-bind="item.label_tag_cont"></td>

                    <td data-th="語系" ng-bind="item.lang.lang_word"></td>
                    <td data-th="排序">
                        <input type="text" class="use-form-control maxWidth" ng-model="item.label_order" ng-blur="contCtrl.changeOrder(item)" ng-focus="contCtrl.textRecording(item)">
                    </td>
                </tr>
            </tbody>
        </table>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" ng-if='contCtrl.actionType == "add"'>新增ICON</h4>
                        <h4 class="modal-title" ng-if='contCtrl.actionType == "edit"'>修改ICON</h4>
                    ...
                    <div class="modal-body">
                        <div class="row use-box">
                            <div class="col-12 use-box-btm">
                                <span class="use-sp-title">ICON名稱</span>
                                <input class="form-control" type='text' ng-model="contCtrl.itemData.label_name" placeholder="請填入ICON名稱">
                            </div>
                            <div class="col-12 use-box-btm">
                                <span class="use-sp-title">圖片</span>
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-group">
                                            <input type="file" class="form-control-file" id="seoFile" ng-file-select="onFileSelect($files)" ng-model="contCtrl.itemData.label_img">
                                        </div>
                                    </div>
                                    <div class="col-6" ng-if='contCtrl.itemData.label_img'>
                                        <div class="adminImg-responsive-4halfBy3" ng-style="{'background-image': 'url('+contCtrl.itemData.label_img+')'}"></div>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 use-box-btm">
                                <span class="use-sp-title">圖片說明</span>
                                <input class="form-control" type='text' ng-model="contCtrl.itemData.label_img_desc" placeholder="請填入圖片說明">
                            </div>
                            
                            <div class="col-12 use-box-btm">
                                <span class="use-sp-title">內容</span>
                                <input class="form-control" type='text' ng-model="contCtrl.itemData.label_tag_cont" placeholder="請填入內容">
                            </div>
                            ...
                            <div class="col-12 use-box-btm">
                                ...
                                <input class="form-control" type='number' ng-model="contCtrl.itemData.label_order" placeholder="請填入順序">
                            </div>


                            <div class="col-12 use-box-btm">
                                <span class="use-sp-title">狀態</span>
                                <select class="form-control" ng-model='contCtrl.selectItemStatus' ng-options='option.name for option in contCtrl.label_status_select' ng-change="contCtrl.itemSatausChange()"></select>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    app.controller('ContentController', ['$scope','$http', function($scope , $http) {
        ...
        // //======= area start=======
        self.listUrl = '/admin/api/prod_label/labelTag/' + self.productNum;
        self.addUrl = '/admin/api/prod_label/labelTag/add';
        self.saveUrl = '/admin/api/prod_label/labelTag/save';
        self.deleteUrl = '/admin/api/prod_label/labelTag/delete';
        ...
        self.resetItem = function() {
            ...
            self.itemData.label_order= 0;
            self.itemData.label_status= 1;
            self.selectItemStatus = self.label_status_select[0];
        }
        ...
        self.edit = function(item) {
            ...
            self.itemData = item;
            self.itemData.label_status ==1 ?  self.selectItemStatus = self.label_status_select[0] :self.selectItemStatus = self.label_status_select[1];
        };
        ...
        self.itemSatausChange = function() {
            self.itemData.label_status = self.selectItemStatus.value;
        };
        ...
        self.getItems = function(search) {
            ...
            $http({
                ...
                angular.forEach(self.items, function(item) {
                    item.label_status ==1 ?item.label_status_name = '啟用' :item.label_status_name = '停用';
                });
        ...
        self.addItem = function() {
            ...
            if(self.itemData.label_name =='' || self.itemData.label_name ==undefined){
                // $('#seoFile').value('');
                $.toaster({ message : '請填入標籤名稱',  priority : 'warning'})
                return false;
            }
            self.itemData.label_prod_num = self.productNum;
        ...
        self.setStatusUrl = '/admin/api/prod_label/labelTag/setStatus';
